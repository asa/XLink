...
XLinkError_t XLinkInitialize(XLinkGlobalHandler_t* globalHandler) {
    ...
    for (i = 0; i < MAX_LINKS; i++) {
        ...
        int stream;
        for (stream = 0; stream < XLINK_MAX_STREAMS; stream++) link->availableStreams[stream].id = INVALID_STREAM_ID;
    }
...
XLinkError_t XLinkFindAllSuitableDevices(const deviceDesc_t in_deviceRequirements, deviceDesc_t* out_foundDevicesPtr,
                                         const unsigned int devicesArraySize, unsigned int* out_foundDevicesCount) {
...
XLinkError_t XLinkConnect(XLinkHandler_t* handler) {
    ...
    mvLog(MVLOG_DEBUG, "%s() device name %s glHandler %p protocol %d\n", __func__, handler->devicePath, glHandler,
          handler->protocol);

    link->deviceHandle.protocol = handler->protocol;
    int connectStatus = XLinkPlatformConnect(handler->devicePath2, handler->devicePath, link->deviceHandle.protocol,
                                             &link->deviceHandle.xLinkFD);
    ...
    XLINK_RET_ERR_IF(DispatcherStart(&link->deviceHandle) != X_LINK_SUCCESS, X_LINK_TIMEOUT);
...
XLinkError_t XLinkResetRemote(linkId_t id) {
    ...
    DispatcherAddEvent(EVENT_LOCAL, &event);
    XLINK_RET_ERR_IF(DispatcherWaitEventComplete(&link->deviceHandle, XLINK_NO_RW_TIMEOUT), X_LINK_TIMEOUT);

    int rc;
    while (((rc = XLink_sem_wait(&link->dispatcherClosedSem)) == -1) && errno == EINTR) continue;
    if (rc) {
...
static linkId_t getNextAvailableLinkUniqueId() {
    ...
    do {
        int i;
        for (i = 0; i < MAX_LINKS; i++) {
            if (availableXLinks[i].id != INVALID_LINK_ID && availableXLinks[i].id == nextUniqueLinkId) break;
        }
        if (i >= MAX_LINKS) {
...
const char* XLinkDeviceStateToStr(XLinkDeviceState_t val) {
    switch (val) {
        ...
        case X_LINK_BOOTED_NON_EXCLUSIVE:
...
const char* XLinkPCIEBootloaderToStr(XLinkPCIEBootloader val) {
    switch (val) {
        case X_LINK_PCIE_UNKNOWN_BOOTLOADER:
            return "X_LINK_PCIE_UNKNOWN_BOOTLOADER";
        case X_LINK_PCIE_SIMPLIFIED_BOOTLOADER:
            return "X_LINK_PCIE_SIMPLIFIED_BOOTLOADER";
        case X_LINK_PCIE_UNIFIED_BOOTLOADER:
            return "X_LINK_PCIE_UNIFIED_BOOTLOADER";
        default:
            return "INVALID_ENUM_VALUE";
            break;
    }
