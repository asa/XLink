...
xLinkPlatformErrorCode_t XLinkPlatformBootFirmware(const deviceDesc_t *deviceDesc, const char *firmware,
                                                   size_t length) {
...
xLinkPlatformErrorCode_t XLinkPlatformCloseRemote(xLinkDeviceHandle_t *deviceHandle) {
    if (deviceHandle->protocol == X_LINK_ANY_PROTOCOL || deviceHandle->protocol == X_LINK_NMB_OF_PROTOCOLS) {
 ...
 */
// UsbSpeed_t get_usb_speed() { return usb_speed_enum; }
...
// int pciePlatformConnect(UNUSED const char *devPathRead, const char *devPathWrite, void **fd) {
...
// Disable sigpipe reception on send
    ...
    if (len == 0 || len >= maxlen + 1) return X_LINK_PLATFORM_INVALID_PARAMETERS;
    char *const serv_ip = (char *)malloc(len + 1);
    if (!serv_ip) return X_LINK_PLATFORM_ERROR;
    serv_ip[0] = 0;
...
// xLinkPlatformErrorCode_t usbPlatformBootBootloader(const char *name) { return usbLinkBootBootloader(name); }
...
xLinkPlatformErrorCode_t tcpipPlatformBootBootloader(const char *name) { return tcpip_boot_bootloader(name); }

/*
static char *pciePlatformStateToStr(const pciePlatformState_t platformState) {
    switch (platformState) {
        case PCIE_PLATFORM_ANY_STATE:
            return "PCIE_PLATFORM_ANY_STATE";
        case PCIE_PLATFORM_BOOTED:
            return "PCIE_PLATFORM_BOOTED";
        case PCIE_PLATFORM_UNBOOTED:
